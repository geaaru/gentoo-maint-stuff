...
gentoo_stage3_init () {
  ...
  gentoo_create_reposfile || return 1

  gentoo_set_best_mirrors || return 1

  gentoo_stage3_libressl_stuff || return 1

  return 0
}

gentoo_stage3_libressl_stuff () {

  # Set CURL_SSL variable
  echo "CURL_SSL=\"libressl\"" >> ${MAKE_PORTAGE_FILE}

  # Set Default USE Flags:
  # NOTE: nettle is needed for permit compilation of iputils (alternative is gcrypt)
  echo "USE=\"libressl nettle -openssl\"" >> ${MAKE_PORTAGE_FILE}

  mkdir -p /etc/portage/profile

  echo "-libressl" >> /etc/portage/profile/use.stable.mask
  echo "dev-libs/openssl" >> /etc/portage/package.mask
  echo "=dev-libs/libressl-2.2*" >> /etc/portage/package.accept_keywords

  return 0
}

gentoo_stage3_libressl_migration () {

  # Libressl migration steps
  # See: https://wiki.gentoo.org/wiki/Project:LibreSSL

  emerge -f libressl || return 1
...
gentoo_stage3_rebuildall () {

  local emerge_opts="-j --newuse --deep --with-bdeps=y"

  gentoo_init_portage || return 1

  gentoo_stage3_libressl_migration || return 1
  ...
  gentoo_set_locale_conf || return 1
